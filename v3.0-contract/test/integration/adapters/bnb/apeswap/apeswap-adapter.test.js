...
describe("Apeswap Adapters Integration Test", function () {
    before("Deploy contract", async function () {
        ...
        const strategy = "0x34878F6a484005AA90E7188a546Ea9E52b538F6f"; // BNB Market
        const swapRouter = "0x10ED43C718714eb63d5aA57B78B54704E256024E"; // pks rounter address
        const name = "Apeswap::Lend::BNB";
        ...
        // Deploy ApeswapBNBLendAdapterBsc contract
        const ApeswapBNBLendAdapterBsc = await setupBscAdapterWithLib("ApeswapLendAdapterBsc", this.lib);
        this.adapter = [0, 0];
        this.adapter[0] = await ApeswapBNBLendAdapterBsc.deploy();
        await this.adapter[0].deployed();
        await this.adapter[0].initialize(strategy, wbnb, swapRouter, name, this.authority.address);
        ...
        await setPath(this.pathFinder, this.pathManager, swapRouter, [wbnb, cake]);
        ...
        console.log("ApeswapLendAdapterBsc: ", this.adapter[0].address);
    ...
    describe("deposit() function test", function () {
        ...
        it("(3) deposit should success for Alice", async function () {
            ...
            expect(aliceInfo.amount).to.be.within(
                BigNumber.from(10).mul(this.bnbPrice).mul(999).div(1e3),
                BigNumber.from(10).mul(this.bnbPrice).mul(1001).div(1e3)
            );
        ...
        it("(4) deposit should success for Bob", async function () {
            ...
            const alicePending = (await this.investor.pendingReward(1, this.alice.address)).withdrawable;
    ...
    describe("deposit() function test after editFund", function () {
        ...
        it("(3) deposit should success for Alice", async function () {
            ...
            const aliceInfo = await this.investor.userInfos(1, this.alice.address);
            expect(aliceInfo.amount).to.be.within(
                BigNumber.from(10).mul(this.bnbPrice).mul(99).div(100),
                BigNumber.from(10).mul(this.bnbPrice).mul(101).div(100)
            );
