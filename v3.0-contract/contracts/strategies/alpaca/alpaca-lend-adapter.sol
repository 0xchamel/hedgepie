...
contract AlpacaLendAdapterBsc is BaseAdapter {
    ...
    function deposit(
        ...
        if (isBNB) {
            IStrategy(aDetail.strategy).deposit{value: amountOut}(amountOut);
        } else {
    ...
    function claim(
        ...
        uint256 wantAmt = (((userInfo.amount * IStrategy(aDetail.strategy).totalToken()) /
            IERC20(aDetail.strategy).totalSupply()) / 1e18);
        uint256 wantShare = ((wantAmt > userInfo.invested ? wantAmt - userInfo.invested : 0) *
            1e18 *
            IERC20(aDetail.strategy).totalSupply()) / IStrategy(aDetail.strategy).totalToken();
